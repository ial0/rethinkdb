...
versioned_t<new_t> migrate_vclock_transform(
    ...
    auto timestamp = realtime_t::min() + chrono::seconds{1 + best_total};
    return versioned_t<new_t>::make_with_manual_timestamp(
        std::chrono::system_clock::to_time_t(timestamp), converter(best_value));
}
    ...
    new_md.replication_info =
        versioned_t<metadata_v1_16::table_replication_info_t>::make_with_manual_timestamp(
            ...
            std::chrono::system_clock::to_time_t(
                realtime_t::min() + chrono::seconds{1} +
                chrono::seconds{blueprint_vclock_total + acks_vclock_total}),
