...
public:
    ...
    void run(chrono::milliseconds timeout_ms) {
        ...
        final_interruptor.start(chrono::seconds{30}); // Allow up to 30 seconds to finish the final verification
...
private:
    void fuzz_states(cond_t *done, signal_t *interruptor) {
        try {
            while (true) {
                ...
                timeout.start(chrono::milliseconds{rng.randint(800)});
    ...
    void fuzz_config(cond_t *done, signal_t *interruptor) {
        try {
            while (true) {
                signal_timer_t timeout;
                timeout.start(chrono::milliseconds{rng.randint(100)});
                wait_interruptible(&timeout, interruptor);
